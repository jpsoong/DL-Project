"""Data loader for the TSP dataset. """
import os
import re
import codecs
import unicodedata
...
class TSPSpeech(Dataset):
    def __init__(self, keys, dir_name='mathias_dataset/'):
        self.keys = keys
        self.path = os.path.join(os.path.dirname(os.path.realpath(__file__)), dir_name)
        print(self.path)
        self.fnames, self.text_lengths, self.texts = read_metadata(os.path.join(self.path, 'mathias_metadata.csv'))
